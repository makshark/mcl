class GamesController < ApplicationController
  ...
  def studliga_rating
    ...
    players_points_count = GamePlayer.joins(:game).where('games.students_league = true AND extract(month from games.date) = 2').group(:player_id).sum(:points)
    ...
    players_points_count.each do |player|
      ...
      game_count = GamePlayer.joins(:game).where('game_players.player_id = (?) ', player[0]).where('games.students_league = true AND extract(month from games.date) = 2').count
      ...
      object[:penalty_amount] = GamePlayer.joins(:game).where('game_players.player_id = (?)', player[0]).where('games.students_league = true AND extract(month from games.date) = 2').sum(:penalty_amount)
    ...
    game_count = (Game.where(students_league: true).where('extract(month from date) = ?', 2).count.to_f / 100) * 20
    position = 1
