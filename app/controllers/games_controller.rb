class GamesController < ApplicationController
  ...
  def studliga_rating
  # .where('extract(month from date) = ?', 11) # example
    @result_array = []
    object = {}
    # TODO: очень быдловское решение, если не исправлю - будет стыдно
    # players_points_count = GamePlayer.group(:player_id)
    #                            .sum(:points)
    players_points_count = GamePlayer.joins(:game).where('games.students_league = true AND extract(month from games.date) = 11').group(:player_id).sum(:points)
    ...
    players_points_count.each do |player|
      ...
      object[:penalty_amount] = GamePlayer.joins(:game).where('game_players.player_id = (?)', player[0]).where('games.students_league = true AND extract(month from games.date) = 11').sum(:penalty_amount)
    ...
    game_count = (Game.where(students_league: true).where('extract(month from date) = ?', 11).count.to_f / 100) * 20
    position = 1
